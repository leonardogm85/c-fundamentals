namespace C.Fundamentos.Modulo15.Exercicio1
{
    /// <summary>
    /// <para>
    ...
    public static class Resposta
    {
        public static void Executar()
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    string? itemExistente = reader.ReadLine();

                    if (!string.IsNullOrWhiteSpace(itemExistente))
                    {
                        Console.WriteLine("Você já possui os seguintes itens na sua lista de compras:");

                        do
                        {
                            Console.WriteLine(itemExistente);
                            itemExistente = reader.ReadLine();
                        }
                        while (!string.IsNullOrWhiteSpace(itemExistente));
